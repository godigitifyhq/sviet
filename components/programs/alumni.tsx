      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_340px]">
        ...
        <div className="space-y-6">
          {ALUMNI_LIST.map((alumni, index) => (
            <div key={alumni.name} className={`flex items-center gap-3 rounded-lg px-4 py-3 ${index === 0 ? "border border-[#f1a866] bg-[#fff4ec]" : "border border-[#e9e9e9] bg-white"}`}>
              <Image
              ...
              <div>
                ...
                <p className="text-xs text-[#666]">{alumni.company} • {alumni.package}</p>
              </div>
            </div>
          ))}
