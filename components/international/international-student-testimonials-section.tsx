...
const TESTIMONIALS = [
  ...
  {
    title: "From academics to campus life, every experience here helped me grow.",
    name: "Khalid Mahmood",
    image: "/assets/img/students/Placement-Mockup-1.png",
  },
  {
    title: "I found mentorship, opportunities, and lifelong friendships at SVIET.",
    name: "Akash Pal",
    ...
    <section className="bg-background py-12 md:py-16">
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <h2 className="text-3xl font-semibold leading-tight text-[#111827] md:text-5xl">
          Here Is What Our
          <br />
          <span className="bg-linear-to-r from-[#2563EB] to-[#f7941d] bg-clip-text font-extrabold text-transparent">International Students Have To Say</span>
        </h2>

        <div className="mt-8 flex gap-4 overflow-x-auto pb-3 no-scrollbar">
          {TESTIMONIALS.map((testimonial) => (
            <article key={testimonial.name} className="relative h-80 w-80 shrink-0 overflow-hidden rounded-2xl border border-black/10 bg-white p-5 md:w-88">
              <div className="text-4xl font-bold text-[#f7941d]">&quot;</div>
              <p className="mt-2 text-base font-semibold leading-snug text-[#111827]">{testimonial.title}</p>
              <p className="mt-2 text-sm text-[#6b7280]">{testimonial.name}</p>

              <div className="absolute bottom-0 left-0 right-0 flex items-end justify-between p-5">
                <div className="relative h-28 w-24">
                  <Image src={testimonial.image} alt={testimonial.name} fill sizes="96px" className="object-contain object-bottom" />
                </div>
