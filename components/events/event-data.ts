...
import type { EventCard, EventRecord, EventSpeaker } from "@/components/events/types";
...
function toCard(event: EventRecord, index: number): EventCard {
    return {
        ...
        image: normalizeText(event.headerImage, FALLBACK_IMAGES[index % FALLBACK_IMAGES.length]),
...
export const EVENT_CARDS: EventCard[] = (eventsData as EventRecord[]).map(toCard);
