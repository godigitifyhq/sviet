  ...
  return (
    <section className="w-full  py-14 md:py-18">
      <div className="mx-auto w-full max-w-360 px-4 md:px-6">
        <header id="campus-festivals" className="mx-auto max-w-5xl scroll-mt-36 text-center md:scroll-mt-44">
          ...
          <p className="mt-4 text-base leading-relaxed text-[#2e2e2e] md:text-lg">
            Experience the magic of SVIET&apos;s annual festivals, from Navratri, Diwali, Holi, and Christmas celebrations to
            theme-based cultural events. Each festival brings a new wave of joy and unforgettable moments.
          </p>
        </header>
          ...
          {FEST_CARDS.map((card) => (
            ...
            >
              ...
              <div className="absolute inset-x-4 bottom-4 rounded-xl border border-white/15 bg-black/25 p-6 md:inset-x-5 md:bottom-5 md:p-8">
                <h3 className="text-lg font-semibold leading-tight text-white md:text-2xl">{card.title}</h3>
                <p className="mt-1 text-xs leading-relaxed text-white/85 md:text-sm">{card.subtitle}</p>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="mt-12 grid items-center gap-8 md:mt-16 md:grid-cols-[1.15fr_0.85fr]">
          <div id="campus-convocation" className="scroll-mt-36 md:scroll-mt-44">
            <h3 className="text-4xl font-medium leading-tight text-[#151515] md:text-5xl">
              The Grand Send-Off
              <br />
              <span className="bg-linear-to-r from-[#1d4ed8] to-[#f7941d] bg-clip-text font-semibold text-transparent">Convocation</span>
            </h3>
            <p className="mt-5 text-base leading-relaxed text-[#2f2f2f] md:text-lg">
              The annual Convocation Ceremony at SVIET is not just the end of an academic chapter; it is a monumental,
              grand-scale celebration that marks the beginning of your professional journey and a powerful tradition of
              excellence.
            </p>
            <p className="mt-4 text-base leading-relaxed text-[#2f2f2f] md:text-lg">
              Each year, the Convocation honors a massive graduating cohort with over 12,000 students across diverse
              disciplines, with nationally and internationally renowned chief guests and guests of honour.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-2 rounded-xl bg-white p-2">
            <div className="col-span-2 overflow-hidden rounded-lg">
              <Image src="/assets/img/campus-life/r2c2.png" alt="Convocation ceremony" width={900} height={500} className="h-44 w-full object-cover md:h-52" />
            </div>
            <div className="overflow-hidden rounded-lg">
              <Image src="/assets/img/campus-life/image4.png" alt="Convocation graduates" width={500} height={500} className="h-30 w-full object-cover md:h-36" />
            </div>
            <div className="overflow-hidden rounded-lg">
              <Image src="/assets/img/campus-life/r3c2.png" alt="Convocation celebration" width={500} height={500} className="h-30 w-full object-cover md:h-36" />
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 overflow-hidden rounded-2xl bg-linear-to-r from-[#0f2a6d] to-[#1d4ed8] p-6 text-white md:mt-16 md:p-8">
          <div className="grid items-center gap-6 md:grid-cols-[1fr_220px]">
            <div>
              <p className="text-lg font-semibold text-white/85 md:text-xl">Never get lost</p>
              <h4 className="mt-2 text-3xl font-semibold leading-tight md:text-4xl">Connect with the world of SVIET on Instagram</h4>
              <p className="mt-3 text-sm text-white/85 md:text-base">Click the link or scan QR code to connect with us</p>
            ...
            <div className="mx-auto h-44 w-44 rounded-xl bg-white p-3 md:h-52 md:w-52">
              <div className="grid h-full w-full grid-cols-8 gap-0.5">
                {Array.from({ length: 64 }).map((_, index) => {
                  const filled = [0, 1, 2, 5, 6, 7, 8, 10, 13, 15, 16, 18, 21, 23, 24, 26, 29, 31, 32, 34, 37, 39, 40, 42, 45, 47, 48, 50, 53, 55, 56, 57, 58, 61, 62, 63].includes(index);
                  return <span key={index} className={`rounded-[1px] ${filled ? "bg-black" : "bg-white"}`} />;
                })}
              </div>
